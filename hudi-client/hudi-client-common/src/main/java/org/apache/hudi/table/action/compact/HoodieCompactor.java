...
public abstract class HoodieCompactor<T extends HoodieRecordPayload, I, K, O> implements Serializable {
  ...
  HoodieCompactionPlan generateCompactionPlan(
    ...
    LOG.info("Total number of compactionPlan operations " + compactionPlan.getOperations().size());
    if (compactionPlan.getOperations().size() < operations.size()) {
      LOG.warn("compactionPlan not contains all operations. maybe COMPACTION_TARGET_IO is too small."
          + " compactionPlan size: " + compactionPlan.getOperations().size() + " , all operations: " + operations.size());
    }
